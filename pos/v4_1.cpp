...
//						}
					...
					else if(finding_time != 0 && aaction == sstate_::rotations && tick-finding_time>4040)//rotate 2000ms
					{
						finding_time = 0;
					}
...
void actionTarget(const sstate_& taction)
{
	...
	if(L_pid->getIsAcc()==false && (pastAction!=taction) && aaction!=backwards)
	{
		L_pid->setIsAcc(true);
		R_pid->setIsAcc(true);
		if(aaction!=rotations && aaction!=chases)
		{
			L_pid->addErrorAcc(5000);
