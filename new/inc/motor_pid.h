...
public:
	...
	Motor_PID() :
			kP(0), kI(0), kD(0), errorSumBound(0), max(0), min(0), preError(0), errorSum(
					0), target(0), errorIgnore(70), errorAcceptRate(0.4), errorAcc(
					0), isAcc(false), numOfLower(0), errorAccept(0), isCount(
					false) {
	}

	Motor_PID(const float& p, const float& i, const float& d, int m_max,
			int m_min) :
			kP(p), kI(i), kD(d), errorSumBound(0), max(m_max), min(m_min), preError(
					0), errorSum(0), target(0), errorIgnore(70), errorAcceptRate(
					0.4), errorAcc(0), isAcc(false), numOfLower(0), errorAccept(
					0), isCount(false) {
	}
