...
//const float O_X_LEFT_INC_RATIO = 1.0
//		* (((O_X_RIGHT - CARX) + O_NX_OFFSET) - (CARX - O_X_LEFT))
//		/ (CARX - O_X_LEFT);
//
////approach
...
//inline float BeaconApproachAngleCalculate(const uint16_t& bx,
//		const uint16_t& by);
//inline void setAnglePower(const float& radAngle);
//inline void pid();
//inline void actionTarget(const sstate_& taction);
//inline void moveCount(const int& cmDis, const sstate_& nowA,
//		const sstate_& nextA);
//
//int main(void) {
//	System::Init();
...
////
//	auto bFunction =
//			[&menu,&aaction,&seen,&changeSpeedTime,&encoder1,&encoder2](const uint8_t id) {
...
//				pid();
//				if (aaction == chases) {
//					if ((abs(L_pid->getTarget()) > 20 && abs(L_count) < 20)
//							|| (abs(R_pid->getTarget()) > 20
//									&& abs(R_count) < 20)) {
...
//								if (o_target.target->center.first > CARX)
//									moveCount(-30, sstate_::backwards,
//											turnLefts);
//								else
//									moveCount(-30, sstate_::backwards,
//											turnRights);
//							} else
//								moveCount(-60, sstate_::backwards, searchs);
//
//							aaction = backwards;
//							actionTarget(aaction);
//							ssend(aaction);
...
//					} else
//						chases_crash_time = 0;
//				} else {	//chases not depend on changeSpeedTime
//					first_chases_time = 0;
//					chases_crash_time = 0;
//				}
//
//				//nullturn
//				//				if (aaction == chases && tick - first_chases_time > 200
//				//						&& (L_pid->getNumError() > crash_cycle
//				//								|| R_pid->getNumError() > crash_cycle)) {
//				//					Dir_pid->reset();
//				//					L_pid->reset();
//				//					R_pid->reset();
//				//
...
//				if (aaction != backwards && aaction != rotations
//						&& aaction != searchs && aaction != chases
//						&& tick - changeSpeedTime > 200
//						&& ((abs(L_pid->getTarget()) > 20 && abs(L_count) < 20)
//								|| (abs(R_pid->getTarget()) > 20
//										&& abs(R_count) < 20))) {
//					if (aaction == avoids || aaction == forwards
//							|| aaction == approachs) {
////						moveCount(-30, sstate_::backwards, sstate_::forwards); //avoid again if seen obstacle
////						aaction = backwards;
//						int temp_speed = tick - o_target.received_time < 200?-40:-60;
//						if (abs(L_count) > abs(R_count)) {
//							moveCount(temp_speed, sstate_::backwards,
//									sstate_::turnLefts);
//							aaction = backwards;
//						} else {
//							moveCount(temp_speed, sstate_::backwards,
...
//
//					continue;
//				} else if (aaction == sstate_::backwards
//						&& tick - changeSpeedTime > 200
//						&& ((!(abs(L_pid->getTarget()) < 20)
//								&& (abs(L_count) < 20))
//								|| (!(abs(R_pid->getTarget()) < 20)
//										&& (abs(R_count) < 20)))) {
//					if (abs(L_count) > abs(R_count)) {
...
////					}
//
//					if (aaction == sstate_::avoids || aaction == forwards
//							|| aaction == approachs) {
////						moveCount(-30, sstate_::backwards, sstate_::forwards); //avoid again if seen obstacle
////						aaction = backwards;
//						if (tick - o_target.received_time < 200) {
//							if (abs(L_count) > abs(R_count)) {
//								moveCount(-40, sstate_::backwards,
//										sstate_::turnLefts);
//								aaction = backwards;
//							} else {
//								moveCount(-40, sstate_::backwards,
...
//						} else {
//							if (abs(L_count) > abs(R_count)) {
//								moveCount(-60, sstate_::backwards,
//										sstate_::turnLefts);
//								aaction = backwards;
//							} else {
//								moveCount(-60, sstate_::backwards,
...
//						actionTarget(aaction);
//						ssend(aaction);
//					} else if (aaction != sstate_::forwards
//							&& ((L_pid->getNumError() > crash_cycle
//									&& L_pid->getTarget() < 0)
//									|| (R_pid->getNumError() > crash_cycle
//											&& R_pid->getTarget() > 0))) {
...
////					))
//				if ((aaction == sstate_::rotations
//						|| aaction == sstate_::searchs)
//						&& tick - changeSpeedTime > 1000
//						&& ((L_pid->getNumError() > crash_cycle
//								|| R_pid->getNumError() > crash_cycle))) {
////					if(abs(L_count)>abs(R_count))
//					{
...
////otarget/////////////////////////////
//				if (tick - o_target.received_time < 200
//						&& o_target.target->center.first > 40
//						&& o_target.target->center.first < 170
//						&& aaction != sstate_::rotations && aaction != searchs
//						&& aaction != backwards
//						&& !(aaction == chases && avoidCrashWhileChase == false)) {
...
//
//				if (ir_target != NULL
//						&& tick - ir_target2.received_time < 200) {
...
//							R_pid->reset();
//						}
//						setAnglePower(BeaconApproachAngleCalculate(x, y));
//						aaction = approachs;
//						actionTarget(aaction);
//						//changeSpeedTime = System::Time();
//					}
...
//						actionTarget(aaction);
//						finding_time = tick;
//					} else if (finding_time == 0
//							&& aaction == sstate_::forwards) {
//						aaction = rotations;
//						actionTarget(aaction);
//						finding_time = tick;
//					} else if (finding_time != 0
//							&& aaction == sstate_::rotations
//							&& tick - finding_time > 3040) //rotate 2000ms
//									{
//						finding_time = 0;
//					} else if (finding_time != 0
//							&& aaction != sstate_::rotations) //unknown
//									{
//						finding_time = 0;
//					}
//
...
//
////menu///////////////////////////////////
//			if (run == false && tick - pid_time > 200
//					&& menu.GetFlag() == false) {
//				char temp[20];
...
//				if (dy > 0) {
//					//turn right
//					int16_t nbx = (((bx - O_X_LEFT) * (1 + O_X_LEFT_INC_RATIO))
//							+ O_NX_LEFT) - O_NX_OFFSET;
...
//			aCountL += L_count;
//			aCountR -= R_count;
//			int32_t d = abs(accDis);
//			//move dist
//			if (accAction == aaction) {
//				if (abs(aCountL) >= d || abs(aCountR) >= d) { //move forward
//					accCount = false;
//					if (aaction == chases) {
//						Dir_pid->reset();
//						L_pid->reset();
...
//					if (accDis > 0
//							&& (actionAfterMove == turnRights
//									|| actionAfterMove == turnLefts)) {
//						L_pid->setIsAcc(true);
//						R_pid->setIsAcc(true);
...
//		}
//	}
//	if (tick - changeSpeedTime > 200 && L_pid->getIsAcc() == true
//			&& aaction != chases && aaction != rotations
//			&& aaction != searchs) {
//		L_pid->setIsAcc(false);
//		R_pid->setIsAcc(false);
//	}
//
//	if ((tick - changeSpeedTime > 200 && aaction != chases)
//			|| (aaction == chases && tick - first_chases_time > 200)) {
//		L_pid->setIsCount(true);
//		R_pid->setIsCount(true);
//	}
...
//	int32_t pastLpid = L_pid->getTarget();
//	int32_t pastRpid = R_pid->getTarget();
//
//	switch (taction) {
//	case forwards:
//		L_pid->settarget(finding_speed);
...
//	}
//
//	if (pastLpid != L_pid->getTarget() || pastRpid != R_pid->getTarget()) {
//		changeSpeedTime = System::Time();
//		if (aaction != chases) {
//			L_pid->setIsCount(false);
//			R_pid->setIsCount(false);
...
//			&& (aaction != chases || aaction != rotations || aaction == chases)) {
//		L_pid->setIsAcc(false);
//		R_pid->setIsAcc(false);
//	}
//
...
//}
//
//void moveCount(int cmDis, sstate_ nowA, sstate_ nextA) {
...
inline float BeaconApproachAngleCalculate(const uint16_t& bx,
		const uint16_t& by);
inline void setAnglePower(const float& radAngle, const uint32_t& tick,
		uint32_t& pid_time);
inline void pid(const uint32_t& tick, uint32_t& pid_time);
inline void actionTarget(const sstate_& taction);
inline void moveCount(const int32_t& cmDis, const sstate_& nowA,
		const sstate_& nextA);
inline void image();
...
//	menu.SetFlash(&flash0);
//
	auto bFunction = [&menu](const uint8_t id) {
...
//avoid + pid///////////////////////
			if ((tick - last_avoid_time >= 10 || tick - pid_time >= 10)
					&& run == true) {
				////avoid//////////////////////////////
				if (tick - last_avoid_time >= 10 && atom == false
						&& menu.GetFlag() == false) {
///chases crash///////////////////
					//chases not depend on changeSpeedTime
					if (aaction != chases) {
						first_chases_time = 0;
						chases_crash_time = 0;
					}
					...
					//crash
					if (run == true && aaction == sstate_::chases
							&& ((!(abs(L_pid->getTarget()) < 20)
									&& (abs(L_count) < 20))
									|| (!(abs(R_pid->getTarget()) < 20)
											&& (abs(R_count) < 20)))) {
						if (chases_crash_time == 0) {
							...
							pid(tick, pid_time);
							last_avoid_time = tick;
							continue;
						}

					} else if (run == true && aaction == chases
							&& ((!(abs(L_pid->getTarget()) < 20)
									&& (abs(L_count) > 20))
									|| (!(abs(R_pid->getTarget()) < 20)
											&& (abs(R_count) > 20)))) {
					...
					//nullturn
					if (run == true && aaction == chases
							&& tick - first_chases_time > 200
							&& (L_pid->getNumError() > crash_cycle
									|| R_pid->getNumError() > crash_cycle)) {
						Dir_pid->reset();
						L_pid->reset();
...
////crash///////////////////////////////////////////////////////
					//				if(run==true && aaction!= sstate_::backwards && tick - o_target.received_time < 200 && tick-changeSpeedTime>200 && (
					if (run == true && aaction != sstate_::backwards
							&& tick - changeSpeedTime > 200
							&& ((!(abs(L_pid->getTarget()) < 20)
									&& (abs(L_count) < 20))
									|| (!(abs(R_pid->getTarget()) < 20)
						...
						last_avoid_time = tick;
						continue;
					} else if (run == true && aaction == sstate_::backwards
							&& tick - changeSpeedTime > 200
							&& ((!(abs(L_pid->getTarget()) < 20)
									&& (abs(L_count) < 20))
									|| (!(abs(R_pid->getTarget()) < 20)
						...
						if (abs(L_count) > abs(R_count)) {
							//moveCount(30, sstate_::turnLefts, sstate_::forwards);
							moveCount(30, sstate_::turnLefts,
									sstate_::rotations);
							aaction = turnLefts;
						} else {
							//moveCount(30, sstate_::turnRights, sstate_::forwards);
							moveCount(30, sstate_::turnRights,
									sstate_::rotations);
							aaction = turnRights;
						}
						actionTarget(aaction);
...
///null turn/////////////////
					//else
					if (run == true && tick - changeSpeedTime > 200
							&& (L_pid->getNumError() > crash_cycle
									|| R_pid->getNumError() > crash_cycle)
							&& aaction != rotations && aaction != searchs
							&& aaction != chases) {
...
//otarget/////////////////////////////
					if (tick - o_target.received_time < 200
							...
							&& o_target.target->center.first < 170
							&& aaction != sstate_::rotations
							&& aaction != searchs && aaction != backwards
							&& !(aaction == chases
									&& avoidCrashWhileChase == false)) {
						uint16_t x = o_target.target->center.first;
						uint16_t y = o_target.target->center.second;
						led0->SetEnable(true);
						if (printLCD) {
...
//process///////////////////////////
			if (tick - process_time >= 29 && atom == false
				...
				/**
				 ...
				 //nullturn
				 if(run==true && aaction==chases && tick-first_chases_time>200 && (L_pid->getNumError()>crash_cycle || R_pid->getNumError()>crash_cycle))
				 {
				 Dir_pid->reset();
				 L_pid->reset();
				 R_pid->reset();
				 ...
				 {
				 //moveCount(30, sstate_::turnLefts, sstate_::forwards);
				 moveCount(30, sstate_::turnLefts, sstate_::rotations);
				 aaction = turnLefts;
				 }
				 else
				 {
				 //moveCount(30, sstate_::turnRights, sstate_::forwards);
				 moveCount(30, sstate_::turnRights, sstate_::rotations);
				 aaction = turnRights;
				 }
				 actionTarget(aaction);
				 //					finding_time = 0; //trigger rotations in not seen condition check
				 ...
				 //otarget/////////////////////////////
				 if (tick - o_target.received_time < 200 && o_target.target->center.first > 40 && o_target.target->center.first < 170
				 && aaction!=sstate_::rotations && aaction!=searchs && aaction!=backwards
				 && !(aaction==chases && avoidCrashWhileChase==false))
				 {
				 uint16_t x = o_target.target->center.first;
				 uint16_t y = o_target.target->center.second;
...
//						}
					...
					} else if (needRot == false && aaction == rotations
							&& tick - finding_time > 3040) {
						if (aaction == chases) {
							Dir_pid->reset();
							L_pid->reset();
							R_pid->reset();
...
void setAnglePower(const float& radAngle, const uint32_t& tick,
		uint32_t& pid_time) {
	if (radAngle >= 2) {
		return;
	}
...
void pid(const uint32_t& tick, uint32_t& pid_time) {
	...
	if (tick - changeSpeedTime > 200 && aaction != chases) {
		L_pid->setIsCount(true);
		R_pid->setIsCount(true);
	} else if (aaction == chases && tick - first_chases_time > 200) {
		L_pid->setIsCount(true);
		R_pid->setIsCount(true);
	}
...
void moveCount(const int32_t& cmDis, const sstate_& nowA,
		const sstate_& nextA) {
	accDis = (cmDis * COUNT_PER_CM);
	accCount = true;
	firstAcc = true;
		...
		if (System::Time() != tick) {
			...
			if (beacon_count > 0)
				for (int i = 0; i < beacon_count; i++) {
					...
					sprintf(data, "%d: %d", i, beacons[beacon_count].density);
