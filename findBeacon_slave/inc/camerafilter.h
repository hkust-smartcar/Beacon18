...
int threepartpixel (bool inputarray[],int width,int height ,St7735r & lcd,LcdTypewriter & writer){
	int leftp = 0;//1
	int rightp =0;//2
	int midp=0;//3
	...
	lcd.FillColor(lcd.kGray);
	lcd.SetRegion(Lcd::Rect(5,height/2,3,height/2));
	lcd.FillColor(lcd.kGray);
	lcd.SetRegion(Lcd::Rect(width/3,height/2,3,height/2));
	lcd.FillColor(lcd.kGray);
	lcd.SetRegion(Lcd::Rect(width*2/3-10,height/2,3,height/2));
	lcd.FillColor(lcd.kGray);
	...
	lcd.SetRegion(Lcd::Rect(0,100,width,10));
	char data[20] = { };
