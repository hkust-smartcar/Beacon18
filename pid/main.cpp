	...
	while (1) {
		if (tick != System::Time() && g_start) {
			...
			if (tick % 15 == 0) {
				...
				char v[10] = { };
				sprintf(v, "l:%d/tr:%d", GetMotorPower(0),GetMotorPower(1));
...
//						L_pid->settarget(100);
//						R_pid->settarget(150);
						L_pid->settarget((int)(ltarget*0.67));
						R_pid->settarget(rtarget);
					} else if (move[3]) {
//						L_pid->settarget(150);
//						R_pid->settarget(100);
						L_pid->settarget(ltarget);
						R_pid->settarget((int)(rtarget*0.67));
					} else {
//						L_pid->settarget(100);
//						R_pid->settarget(100);
						L_pid->settarget(ltarget);
